...
export default function StakingSection() {
    ...
    const [selectedOption, setSelectedOption] = useState<string>(StakingOptions[0].value);
    ...
    return (
        <div className="relative z-10 flex w-full flex-col items-center px-4 py-5 sm:px-6 sm:pt-0 lg:px-8">
                ...
                <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5">
                    ...
                    <KartCard className="gap-3">
                        <div className="flex w-full flex-col items-center gap-y-2 sm:items-start">
                            ...
                            <div className="text-sm font-light text-gray-300">19,822,172.065 staked</div>
                        </div>
                        <button className="w-full bg-purple px-2 py-2 text-white text-xs rounded-lg">Staked / Circulating</button>
                    </KartCard>
                </div>
            </main >
            <KartCard className="my-10 flex w-full max-w-lg flex-col items-start gap-y-8 p-8 drop-shadow-16 sm:mb-0">
                ...
                <div className="flex w-full space-x-2 rounded-sm bg-transparent p-2 transition-all duration-200 ease-in-out">
                    {
                        StakingOptions.map((item, index) => (
                            <button key={index} className={`w-full rounded-sm py-2 border transition-all duration-200 ease-in-out bg-purple text-gray-300 ${selectedOption === item.value ? "bg-purple border-transparent" : "bg-transparent border-purple-border"}`} onClick={() => setSelectedOption(item.value)}>{item.label}</button>
                        ))
                    }
                </div>
                        ...
                        <div className="mt-2 flex items-center rounded-sm bg-transparent px-4 py-2 shadow-sm border border-purple-border">
                            <div className="relative flex grow items-stretch">
                                <div className="pointer-events-none absolute inset-y-0 left-0 flex items-center">
                                    <Image alt="Kart Logo black" width={20} height={20} src="/images/logo.png" />
                                </div>
                            ...
                            <button type="button" className="relative text-gray-300 inline-flex items-center gap-x-1.5 rounded bg-purple px-2 py-1.5 text-sm shadow-md">MAX</button>
                        </div>
                    </div>
                </div>
                ...
                <button className={`bg-purple border w-full rounded py-3 text-gray-300 transition-all duration-100 ease-in-out ${selectedOption === StakingOptions[0].value ? "bg-purple border-transparent" : "bg-transparent border-purple-border"}`}>{selectedOption === StakingOptions[0].value ? "Stake" : "Unstake"}</button>
            </KartCard>
